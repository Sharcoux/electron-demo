...
function register () {
  ...
  ipcMain.on('start-speech2text', (event, sampleRate) => {
    const model = new vosk.Model('./models/en-US')
  ...
  ipcMain.on('activate-microphone', (event) => {
    console.log(systemPreferences.askForMediaAccess)
    systemPreferences.askForMediaAccess ? systemPreferences
      .askForMediaAccess('microphone')
      .then((result) => event.reply('activate-microphone-response', result))
      : event.reply('activate-microphone-response', true)
  })
}
